...
{
    public partial class ucDecimal : UserControl
    {
        public ucDecimal()
        {
            InitializeComponent();
        ...
        private string separador = System.Globalization.NumberFormatInfo.CurrentInfo.CurrencyDecimalSeparator;
        ...
        public decimal? Valor
        {
            get
            {
                if (string.IsNullOrEmpty(TextBox1.Text))
                    return null;
                return Convert.ToDecimal(TextBox1.Text.Replace(".", ""));
            }
            set
            {
                if (value != null)
...
//            TextBox1.Select();
        ...
        public bool Valido()
        {
            ...
            decimal result;
            var isDecimal = decimal.TryParse(TextBox1.Text.Trim(), out result);
            if (!isDecimal)
            {
            ...
            if (result < minValue)
            {
                ErrorMessage = "El valor ingresado debe ser mayor a " + minValue.ToString("N2");
                return false;
            }
            if (result > maxValue)
            {
                ErrorMessage = "El valor ingresado debe ser menor a " + maxValue.ToString("N2");
                return false;
            }
            return true;
        }
        ...
        private void TextBox1_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (char.IsDigit(e.KeyChar) || e.KeyChar == char.Parse(separador) || e.KeyChar != 8)
            {
                e.Handled = false;
                // se verifica que el primer digito ingresado no sea un punto al seleccionar
                if (TextBox1.SelectedText != "")
                {
                    if (e.KeyChar == char.Parse(separador))
                    {
                        e.Handled = true;
                    }
                }
                if (dig == 1 && e.KeyChar == char.Parse(separador))
                {
                    e.Handled = true;
                }
                ...
                if (TextBox1.SelectedText == "")
                {
                    ...
                    for (a = 0; a <= dig - 1; a++)
                    {
                        string car = (TextBox1.Text + e.KeyChar).ToString();
                        if (car.Substring(a, 1) == separador)
                        {
                            esDecimal++;
                            esDec = true;
                        }
