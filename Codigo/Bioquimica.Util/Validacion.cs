            ...
            foreach (var control in controles)
            {
                switch (control.GetType().ToString())
                {
                    ...
                    case "Util.Controles.ucSoloNumero":
                        ...
                        break;
                    case "Util.Controles.ucPorcentaje":
                        var ucPorcentaje = (ucPorcentaje)control;
                        if (!ucPorcentaje.Valido())
                        {
                            provider.SetError(ucPorcentaje, ucPorcentaje.ErrorMessage);
                            provider.SetIconPadding(ucPorcentaje, 2);
                            valido = false;
                        }
                        break;
                    ...
                    case "Util.Controles.ucPassword":
                        ...
                        break;
                    case "Util.Controles.ucDecimal":
                        var ucDecimal = (ucDecimal)control;
                        if (!ucDecimal.Valido())
                        {
                            provider.SetError(ucDecimal, ucDecimal.ErrorMessage);
                            provider.SetIconPadding(ucDecimal, 2);
                            valido = false;
                        }
                        break;
                }
